...
export class DataService {
  ...
  getTranscations(filterParam: FilterParam) {
    ...
    // if (filterParam) {
    //   params = params.append('?', `${filterParam}`);
    // }

    // console.log(apiConfig.transaction_url + `${filterParam}`);

    return this.httpClient.get(apiConfig.transaction_url + `${filterParam}`).pipe(
