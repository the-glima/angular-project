  ...
  ngOnInit() {
    this.dropdownService.dropdownOptionSelected.subscribe(
      (dropdown: Dropdown) => {
        ...
        console.log('FROM FILTER: ' + this.filters);

        this.filters.push(filterItem);
        const foundIndex = this.findFilter(this.filters, filterItem.filterId);

        if (foundIndex){
          this.filters[foundIndex] = item;
        }
      }
    );
  }
